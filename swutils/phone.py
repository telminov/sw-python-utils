...
def gen_canonical_phone(original_phone, first_number='7', check_code=True):
    ...
    if not (10 <= len(phone) <= 11):
        raise CanonicalPhoneGenerationException(u'Номер должен состоять из 10 или 11 цифр (%s)' % str(original_phone))

    # избавимся от ведущей цифры 11ти значного номера
    phone = phone[-10:]

    if check_code and phone[0] != '9':
        raise CanonicalPhoneGenerationException(u'Код города должен начинатся с 9 (%s)' % str(original_phone))
